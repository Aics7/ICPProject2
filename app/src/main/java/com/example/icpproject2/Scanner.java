...
public class Scanner extends AppCompatActivity implements ZXingScannerView.ResultHandler {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(currentApiVersion >=  Build.VERSION_CODES.M)
        {
            if(checkPermission())
            {
                Toast.makeText(getApplicationContext(), "Access granted already", Toast.LENGTH_LONG).show();
            }
            else
            {
                requestPermission();
        ...
        switch (requestCode) {
            case REQUEST_CAMERA:
                if (grantResults.length > 0) {
                    ...
                    if (cameraAccepted){
                        Toast.makeText(getApplicationContext(), "Access Granted, Ready to start scan", Toast.LENGTH_LONG).show();
                    }else {
                        Toast.makeText(getApplicationContext(), "Access Denied, This section needs camera access to operate", Toast.LENGTH_LONG).show();
    ...
    public void handleResult(Result result) {
        ...
        builder.setTitle("Scan Result and Payment confirmation");
        ...
        builder.setNeutralButton("Make Payment", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                Intent int1=new Intent(Scanner.this,MakePayment.class);
                startActivity(int1);
            }
        });
